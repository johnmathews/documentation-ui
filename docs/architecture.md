...
- **Sidebar**: Tree navigation organized by Source > Category (root\_docs/docs/journal/engineering\_team) > Document, with expand/collapse controls inside the tree section. Each source has a deterministic color tag for visual distinction. Root-level documents (e.g. README.md) appear directly under each source, before subcategories. A collapsible "Filter categories" section with GOV.UK-style small checkboxes allows globally toggling category visibility (persisted to localStorage). The `CATEGORIES` constant in `stores.svelte.ts` is the single source of truth for category definitions.
